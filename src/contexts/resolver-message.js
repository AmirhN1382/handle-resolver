...
module.exports = {
	data: new ContextMenuCommandBuilder()
		.setName('resolve-message')
		.setType(ApplicationCommandType.Message),
	async execute(interaction) {
		await interaction.deferReply({ephemeral: false});
		let handleRegex = new RegExp(config.handle_regex, 'i');
		if(!handleRegex.test(interaction.targetMessage.content)) return interaction.editReply({embeds: [SEG.JustDescp(`No Handle Found.`, `Red`)]});
		interaction.targetMessage.content = interaction.targetMessage.content.toLowerCase();
		let handle = handleRegex.exec(interaction.targetMessage.content);
